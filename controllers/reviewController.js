...
export const createReview = async (req, res) => {
    ...
    try{
        ...
        if (member.rows.length === 0) {
            logger.warn('User is not a member of the club');
            return res.status(403).json({ error: 'User is not a member of the club' });
        } 
            

        const existingReview = await pool.query( 'SELECT * FROM reviews WHERE book_id = $1 AND user_id = $2', [book_id, user_id] ); 
        if (existingReview.rows.length > 0) {
            logger.warn('User has already reviewed this book');
            return res.status(403).json({ error: 'User has already reviewed this book' });
        } 
...
export const updateReview = async (req, res) => { 
    ...
    logger.info('try update review: ${id}');
    try { 
...
export const deleteReview = async (req, res) => { 
    const { id } = req.params; 
    logger.info('Try delete review: ${id}');
    try { 
        const existingReview = await pool.query( 'SELECT * FROM reviews WHERE id = $1', [id] ); 
        if (existingReview.rows.length === 0) {
            logger.warn('Review not found');
