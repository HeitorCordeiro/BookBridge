...
export const getBooks = async (req, res) => {
    try{
        ...
        if(cachedBooks) return res.status(200).json(cachedBooks);
       
        const books = await pool.query('SELECT * FROM clubs');
        await setCache(cacheKey, books.rows);    
